...
use crate::mdm::utils::{
    generate_diff, home_dir, is_git_ai_checkpoint_command, normalize_windows_path_for_shell,
    write_atomic,
};
...
const TRAE_CHECKPOINT_CMD: &str = "checkpoint trae --hook-input stdin";
const TRAE_CATCH_ALL_MATCHER: &str = "*";

/// The TraeCode hook events we install into (see TRAE "Hook 配置详解").
const HOOK_EVENTS: &[&str] = &["PreToolUse", "PostToolUse"];

/// Hook installer for the TRAE (TraeCode) IDE.
///
/// TraeCode reads Claude-Code-style hook configuration from hooks.json files.
/// Both distributions are covered: the CN version (~/.trae-cn) and the
/// international version (~/.trae).
pub struct TraeInstaller;

impl TraeInstaller {
    /// Both global locations where TraeCode looks for hooks.
        ...
        for hook_type in HOOK_EVENTS {
            ...
            let catch_all_idx = hook_type_array
                ...
                .position(|b| {
                    b.get("matcher")
                        .and_then(|m| m.as_str())
                        .map(|m| m == TRAE_CATCH_ALL_MATCHER)
                        .unwrap_or(false)
                })
                .unwrap_or_else(|| {
    ...
    fn uninstall_hooks_at(hooks_path: &Path, dry_run: bool) -> Result<Option<String>, GitAiError> {
        ...
        for hook_type in HOOK_EVENTS {
            ...
            {
                // Remove git-ai commands; remember which blocks we emptied so
                // only those (and not pre-existing empty blocks) are dropped.
                let mut emptied_block_indices = Vec::new();
                for (block_idx, matcher_block) in hook_type_array.iter_mut().enumerate() {
                ...
                for block_idx in emptied_block_indices.into_iter().rev() {
                    hook_type_array.remove(block_idx);
                }
            }
        }
...
impl HookInstaller for TraeInstaller {
    ...
    fn check_hooks(&self, _params: &HookInstallerParams) -> Result<HookCheckResult, GitAiError> {
        let home = home_dir();
        let has_dotfiles = home.join(".trae-cn").exists() || home.join(".trae").exists();
...
mod tests {
    ...
    fn expected_cmd() -> String {
        TraeInstaller::desired_command(&binary_path())
    }
    ...
    #[test]
    fn test_trae_installer_id() {
        assert_eq!(TraeInstaller.id(), "trae");
        assert_eq!(TraeInstaller.name(), "TRAE");
    }
    ...
    fn s1_fresh_install_creates_catch_all_block() {
        ...
        let diff = TraeInstaller::install_hooks_at(&path, &expected_cmd(), false).unwrap();
        assert!(diff.is_some(), "should produce a diff");

        let hooks = read_hooks(&path);
        for event in HOOK_EVENTS {
            let catch_all = hooks_in_catch_all(&hooks, event);
    ...
    fn s2_idempotent_already_installed() {
        ...
        fs::write(
            ...
            serde_json::to_string_pretty(&json!({
                "version": 1,
                "hooks": {
                    "PreToolUse": [{"matcher": "*", "hooks": [{"type":"command","command": cmd, "timeout": 30}]}],
                    "PostToolUse": [{"matcher": "*", "hooks": [{"type":"command","command": cmd, "timeout": 30}]}]
                }
            }))
            .unwrap(),
        )
    ...
    fn s3_install_preserves_user_hooks_and_other_matchers() {
        ...
        TraeInstaller::install_hooks_at(&path, &expected_cmd(), false).unwrap();

        let hooks = read_hooks(&path);

        // PreToolUse: user matcher block untouched, catch-all created with our hook.
        let pre_blocks = hooks
            .get("hooks")
            .and_then(|h| h.get("PreToolUse"))
            .and_then(|v| v.as_array())
            .unwrap();
        assert_eq!(pre_blocks.len(), 2, "user block + catch-all block");
        let run_command_block = pre_blocks
            .iter()
            .find(|b| b.get("matcher").and_then(|m| m.as_str()) == Some("RunCommand"))
            .unwrap();
        assert_eq!(
            run_command_block
                .get("hooks")
        ...
        let post_catch_all = hooks_in_catch_all(&hooks, "PostToolUse");
    ...
    fn s5_install_dedupes_git_ai_hooks() {
        ...
        let hooks = read_hooks(&path);
        let pre_catch_all = hooks_in_catch_all(&hooks, "PreToolUse");
    ...
    fn s6_dry_run_does_not_write() {
        ...
        let diff = TraeInstaller::install_hooks_at(&path, &expected_cmd(), true).unwrap();
        assert!(diff.is_some());
        assert!(!path.exists(), "dry run must not create the file");
    }

    #[test]
    fn s7_uninstall_removes_git_ai_hooks_only() {
        let (_td, path) = setup_test_env();
        let cmd = expected_cmd();
        fs::write(
            &path,
        ...
        let diff = TraeInstaller::uninstall_hooks_at(&path, false).unwrap();
        assert!(diff.is_some(), "should produce a diff");
        ...
        // The PostToolUse catch-all block only held our hook; it must be removed.
        let post_blocks = hooks
            .get("hooks")
    ...
    #[test]
    fn s8_uninstall_no_hooks_is_noop() {
        let (_td, path) = setup_test_env();
        fs::write(
            &path,
            serde_json::to_string_pretty(&json!({
    ...
    #[test]
    fn s9_uninstall_missing_file_is_noop() {
        let (_td, path) = setup_test_env();
        let diff = TraeInstaller::uninstall_hooks_at(&path, false).unwrap();
        assert!(diff.is_none());
    }
